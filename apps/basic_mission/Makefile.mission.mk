...
ifeq ($(PROD),1)
PERIODIC_CALLBACK_IN_SEC ?= 60
else
PERIODIC_CALLBACK_IN_SEC ?= 30
endif
...
MISSION_RTC_RESYNC_PERIOD_SEC ?= 600
else
# Resync every 20 minutes
MISSION_RTC_RESYNC_PERIOD_SEC ?= 1200
endif
...
#MISSION_MESSAGES_TYPE ?= \{RANGE1_PAYLOAD,TELEMETRY_PAYLOAD,APRS_LORAWAN_PAYLOAD,APRS_AX25_PAYLOAD,TELEMETRY_PAYLOAD,EWSS_PAYLOAD,STAT_PAYLOAD}\
...
#MISSION_MESSAGES_TYPE ?= \{RANGE1_PAYLOAD}\

MISSION_MESSAGES_TYPE ?= \{RANGE1_PAYLOAD,TELEMETRY_PAYLOAD,APRS_LORAWAN_PAYLOAD,TELEMETRY_PAYLOAD,APRS_AX25_PAYLOAD,TELEMETRY_PAYLOAD,EWSS_PAYLOAD,TELEMETRY_PAYLOAD,STAT_PAYLOAD}\
...
TINYGS_FREQUENCY ?= $(MESHTASTIC_FREQUENCY)
TINYGS_BANDWIDTH ?= $(MESHTASTIC_BANDWIDTH)
TINYGS_SPREADING_FACTOR ?= $(MESHTASTIC_SPREADING_FACTOR)
TINYGS_CODING_RATE ?= $(MESHTASTIC_CODING_RATE)
TINYGS_SYNCWORD ?= $(MESHTASTIC_SYNCWORD)
TINYGS_PREAMBLE_LEN ?= $(MESHTASTIC_PREAMBLE_LEN)
TINYGS_RF_POWER ?= $(MESHTASTIC_RF_POWER)
...
# CR 4/8
TINYGS_CODING_RATE ?= 4U
TINYGS_SYNCWORD ?= 18
TINYGS_PREAMBLE_LEN ?= 8
TINYGS_RF_POWER ?= 27
...
# -11 est souvent utilisé pour les ballons
MISSION_APRS_CALLSIGN ?= \"FCSUG\"
MISSION_APRS_COMMENT ?= \"Balloon Thingsat\"
