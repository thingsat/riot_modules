...
/*
	...
	printf("Note: Concentrator EUI: 0x%s\n", eui_hex);
*/
...
int lgw_mem_check_rb(uint16_t mem_addr, const uint8_t *data_to_check, uint16_t size, bool fifo_mode) {
    DEBUG_PRINTF("[%s]: addr=0x%x size=%d\n", __FUNCTION__, mem_addr, size);
    ...
    uint8_t* buf = malloc(CHUNK_SIZE_MAX);
...
*/
    ...
    while (sz_todo > 0) {
        ...
        /* do the burst read */
        com_stat += lgw_com_rb(LGW_SPI_MUX_TARGET_SX1302, addr, buf, chunk_size);

		if (memcmp(data_to_check + (chunk_cnt * CHUNK_SIZE_MAX), buf, chunk_size) != 0) {
			DEBUG_PRINTF("ERROR: READEN FIRMWARE MISMATCHED WITH WRITTEN FIRMWARE AT CHUNK #%d\n",chunk_cnt);
	        free(buf);
	        return LGW_REG_ERROR;
		}
