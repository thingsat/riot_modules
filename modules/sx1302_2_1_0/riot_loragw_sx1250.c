...
int sx1250_reg_w(sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain) {
	...
	DEBUG_RW("[%s:%d] rf_chain=%d, op_code=0x%x, size=%d\n" , __FUNCTION__, __LINE__, rf_chain, op_code, size);

    int com_stat;

    /* checking input parameters */
    if (rf_chain >= LGW_RF_CHAIN_NB) {
        printf("[%s:%d] ERROR: INVALID RF_CHAIN\n", __FUNCTION__, __LINE__);
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        printf("[%s:%d] ERROR: COM ERROR DURING RADIO REGISTER WRITE\n", __FUNCTION__, __LINE__);
        return LGW_REG_ERROR;
    } else {
...
int sx1250_reg_r(sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain) {
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        printf("[%s:%d] ERROR: COM ERROR DURING RADIO REGISTER READ\n", __FUNCTION__, __LINE__);
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int sx1250_calibrate(uint8_t rf_chain, uint32_t freq_hz) {
	...
	DEBUG_RW("[%s:%d] rf_chain=%d, freq_hz=%ld\n" , __FUNCTION__, __LINE__, rf_chain, freq_hz);
    ...
    if ((freq_hz > 430E6) && (freq_hz < 440E6)) {
    	...
    	printf("[%s:%d] ERROR: failed to calibrate sx1250 radio, frequency range not supported (%lu)\n", __FUNCTION__, __LINE__, freq_hz);
        return LGW_REG_ERROR;
    }
    ...
    if (TAKE_N_BITS_FROM(buff[2], 4, 1) != 0) {
    	printf("[%s:%d] ERROR: sx1250 Image Calibration Error\n", __FUNCTION__, __LINE__);
        return LGW_REG_ERROR;
    }
...
int sx1250_setup(uint8_t rf_chain, uint32_t freq_hz, bool single_input_mode) {
	DEBUG_RW("[%s:%d] rf_chain=%d, freq_hz=%ld, single_input_mode=%d\n" , __FUNCTION__, __LINE__, rf_chain, freq_hz, single_input_mode);
    ...
    if ((uint8_t)(TAKE_N_BITS_FROM(buff[0], 4, 3)) != 0x02) {
    	printf("[%s:%d] ERROR: Failed to set SX1250_%u in STANDBY_RC mode\n", __FUNCTION__, __LINE__, rf_chain);
        return LGW_REG_ERROR;
    }
    ...
    if ((uint8_t)(TAKE_N_BITS_FROM(buff[0], 4, 3)) != 0x03) {
    	printf("[%s:%d] ERROR: Failed to set SX1250_%u in STANDBY_XOSC mode\n", __FUNCTION__, __LINE__, rf_chain);
        return LGW_REG_ERROR;
    }
    ...
    if (single_input_mode == true) {
    	DEBUG_PRINTF("[%s:%d] INFO: Configuring SX1250_%u in single input mode\n", __FUNCTION__, __LINE__, rf_chain);
        buff[0] = 0x08;
        buff[1] = 0xE2;
        buff[2] = 0x0D;
        err |= sx1250_reg_w(WRITE_REGISTER, buff, 3, rf_chain);
    ...
    if (err != LGW_REG_SUCCESS) {
    	printf("[%s:%d] ERROR: failed to setup SX1250_%u radio\n", __FUNCTION__, __LINE__, rf_chain);
        return LGW_REG_ERROR;
    }
