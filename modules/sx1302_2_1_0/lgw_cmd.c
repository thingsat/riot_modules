...
#define ENDPOINT_DEVADDR (0xfc001234U)
#endif
...
//	From global_conf.json.sx1250.EU868
	...
	for (i = 0; i < LGW_IF_CHAIN_NB-2; i++) {
		_freq_hz_plan[i] = lgw_config->channel_rfchain[i] ?
	...
	if(!lgw_config->lorastd_enable) {
	  printf("INFO: Lora standard channel %i disabled\n", i);
		...
		if(lgw_config->lorastd_implicit_hdr) {
	    	printf("ERROR: Lora standard channel with implicit header is not implemented\n");
			return EXIT_FAILURE;
		}
    ...
    if(!lgw_config->fsk_enable) {
		...
		printf("CH%d \tRadio %s \t%.3f MHz \t%lu Hz \%lu bps datarate (FSK channel)\r\n",
				i,
				ifconf.rf_chain ? "B" : "A",
				ifconf.rf_chain ?
					(double) (lgw_config->fb + ifconf.freq_hz) / 1000000.0 :
...
static int lgw_bench_cmd(int argc, char **argv) {
	...
	while (nb_remaining_packets_to_transmit--) {
		...
		printf(
				"Transmitting LoRa packet (%d/%d) on %.3fMHz [SF%ldBW%d, TXPOWER %lu, PWID %u, PA %s, ToA %lu msec] devaddr: 0x%8x fCntUp: %ld\n",
				nb_packets_to_transmit - nb_remaining_packets_to_transmit,
				nb_packets_to_transmit,
 ...
 */
static const uint32_t _rfpower_to_test[] = { 12, 14, 16, 17, 18, 19, 21, 23, 25, 27 };
...
static int lgw_chantest_cmd(int argc, char **argv) {
	...
	if (argc != 9) {
		puts("usage: lgw_chantest <nbpkt> <sf> <bw> <preamble> <crc> <invert_iq> <size> <pause>");
		puts("nbpkt: Number of packets to transmit before exiting.");
		puts("sf: Spreading factor (7 .. 12).");
		puts("bw: Bandwidth in kHz (125, 250, 500).");
		puts("preamble: Preamble (8 .. 12).");
	...
	while (nb_remaining_packets_to_transmit--) {
		...
		freq_hz_plan_idx = (nb_packets_to_transmit - nb_remaining_packets_to_transmit) % _freq_hz_plan_len;
		pkt.freq_hz = _freq_hz_plan[freq_hz_plan_idx];

		if((nb_packets_to_transmit - nb_remaining_packets_to_transmit) % _freq_hz_plan_len == 0) {
...
static int _lgw_tx_pkt_tx(const struct lgw_pkt_tx_s* pkt) {
	...
	if (x != 0) {
		...
		printf("ERROR: failed to send packet - ");
		printf("count_us: start:%lu end:%lu delta:%lu\n", inst_cnt_us_start, inst_cnt_us_end, inst_cnt_us_delta);
		_lgw_stat.tx_abort++;
...
static int _lgw_tx(
		...
		const uint32_t spreading_factor,
		const uint32_t bandwidth,
		const uint16_t preamble,
		const uint32_t rf_power,
		const bool crc_on,
		const bool invert_pol,
		const uint16_t phypayload_size,
...
static void _printf_rxpkt(struct lgw_pkt_rx_s *rxpkt) {
	...
	const uint32_t delta_count_us = _get_delta_instcnt(count_us, _last_count_us);
	...
	while (!_quit_rx) {
		...
		if (nb_pkt != 0) {
			for (int i = 0; i < nb_pkt; i++) {
				...
				bool is_last_tx_packet = (rxpkt[i].size == last_phypayload_size && memcmp(rxpkt[i].payload, last_phypayload, last_phypayload_size)==0);
