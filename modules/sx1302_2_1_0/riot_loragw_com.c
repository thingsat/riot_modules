...
int lgw_com_wb(uint8_t spi_mux_target, uint16_t address, const uint8_t *data, uint16_t size) {
    ...
    switch (_lgw_com_type) {
        case LGW_COM_SPI:
            ...
            uint8_t *data_for_checking = (uint8_t *)malloc(size);
            lgw_spi_rb(_lgw_com_target, spi_mux_target, address, data_for_checking, size);
            if(memcmp(data,data_for_checking,(size_t)size)==0) {
            	DEBUG_MSG("INFO: CORRECT\n");
            } else {
            	DEBUG_MSG("INFO: DIFFERENT\n");
            }
            free(data_for_checking);
