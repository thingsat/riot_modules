...
#if 1 /* Disabled because it brings latency on USB, for low value. TODO: do this less frequently ? */
    int32_t val;
...
int sx1302_lora_service_modem_configure(struct lgw_conf_rxif_s * cfg, uint32_t radio_freq_hz) {
    ...
    printf("INFO: Freq2TimeDrift SingleSF: Mantissa = %d (0x%02X, 0x%02X), Exponent = %d (0x%02X)\n", mantissa, (mantissa >> 8) & 0x00FF, (mantissa) & 0x00FF, exponent, exponent);
...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    ...
    if(fw_check==NULL) {
        printf("ERROR: can not malloc fw_check\n");
        return LGW_REG_ERROR;
    }
#endif
    ...
    if (memcmp(firmware, fw_check, MCU_FW_SIZE) != 0) {
        printf("ERROR: AGC firmware read/write check failed\n");
        for(int i=0; i<MCU_FW_SIZE; i+=64) {
            DEBUG_PRINTF("%u:",i);
        	DEBUG_HEX(firmware + (i*64), 64);
            //DEBUG_MSG("\n");
            DEBUG_PRINTF("%u:",i);
        	DEBUG_HEX(fw_check + (i*64), 64);
            DEBUG_MSG("\n");
        }
       free(fw_check);
        return LGW_REG_ERROR;
...
#endif
    DEBUG_MSG("AGC firmware successfully checked\n");
...
int sx1302_agc_start(uint8_t version, lgw_radio_type_t radio_type, uint8_t ana_gain, uint8_t dec_gain, bool full_duplex, bool lbt_enable) {
    ...
    printf("AGC: config of PA start delay done\n");
...
int sx1302_arb_load_firmware(const uint8_t *firmware) {
    ...
    //if (memcmp(firmware, fw_check, sizeof fw_check) != 0) {
    if (memcmp(firmware, fw_check, MCU_FW_SIZE) != 0) {
        printf("ERROR: ARB firmware read/write check failed\n");
        for(int i=0; i<MCU_FW_SIZE; i+=64) {
            DEBUG_PRINTF("%u:",i);
        	DEBUG_HEX(firmware + (i*64), 64);
            //DEBUG_MSG("\n");
	...
	if (lgw_mem_check_rb(ARB_MEM_ADDR, firmware, MCU_FW_SIZE, false)) {
		printf ("ERROR: Failed to check the loaded ARB firmware\n");
		return LGW_HAL_ERROR;
	}
#endif
    printf("ARB firmware successfully checked\n");
    ...
    if (val != 0) {
        printf("ERROR: Failed to load ARB firmware: parity error check failed\n");
        return LGW_REG_ERROR;
    }
    printf("ARB firmware loaded\n");
...
int sx1302_arb_start(uint8_t version, const struct lgw_conf_ftime_s * ftime_context) {
    ...
    if (val != version) {
        printf("ERROR: wrong ARB firmware version (%d)\n", val);
        return LGW_REG_ERROR;
    }
    printf("ARB firmware version: %d\n", val);
