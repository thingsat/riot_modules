...
unsigned ARRAY_LEN(can_trx_t **can_devs) {
	unsigned l = 0;
	for(; can_devs[l] != NULL; l++ );
	return l;
}

extern can_trx_t *can_devs[];
//
////#undef MODULE_TJA1042
//#ifdef MODULE_TJA1042
//#include "tja1042.h"
////tja1042_trx_t tja1042 = { .trx.driver = &tja1042_driver,
////                          .stb_pin = TJA1042_STB_PIN
////};
//// REMARK: tja1042 is defined by libcsp
//extern tja1042_trx_t tja1042;
//
//#endif
//
...
static void print_usage(void)
{
    ...
    puts("can sendrtr ifnum can_id length(0..8)");
    ...
    puts("can set_mode");
    puts("can init");
    puts("can start_receivers");
    // TODO     puts("can stop_receivers");
...
static int _init(int argc, char **argv) {
    ...
    if (argc < 3) {
        puts("usage: can init [trx_id]");
        return 1;
    }

    unsigned trx = atoi(argv[2]);
    if (trx >= ARRAY_LEN(can_devs)) {
        puts("Invalid trx_id");
        return 1;
    }

    int res = can_trx_init(can_devs[trx]);
    if (res < 0) {
...
static int _set_mode(int argc, char **argv) {
    ...
    if (argc < 4) {
        puts("usage: can set_mode [trx_id] [mode]");
        puts("modes:");
        puts("\t0: normal mode");
        puts("\t1: silent mode");
        puts("\t2: standby mode");
    ...
    int res = can_trx_set_mode(can_devs[trx], mode);
    if (res < 0) {
...
static int _start_receivers(int argc, char **argv)
{
	(void)argc;
	(void)argv;
...
int can_cmd(int argc, char **argv)
{
    if (argc < 2) {
        print_usage();
        return 1;
    ...
    else if (strncmp(argv[1], "set_mode", 11) == 0) {
        return _set_mode(argc, argv);
    }
    else if (strncmp(argv[1], "init", 11) == 0) {
        return _init(argc, argv);
    }
    else if (strncmp(argv[1], "start_receivers", 11) == 0) {
        return _start_receivers(argc, argv);
    }
    else {
