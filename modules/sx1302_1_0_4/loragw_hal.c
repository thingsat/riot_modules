...
const char lgw_version_string[] = "Version: " LIBLORAGW_VERSION "; " LIBLORAGW_RELEASE_DATE "; (RIOT port)";
...
/**
 * Mutex for avoid SPI read/write interleaving when RX and TX
 */
static mutex_t rx_tx_mutex = MUTEX_INIT;


#if ENABLE_STTS7551 == 1
...
int lgw_rxrf_setconf(uint8_t rf_chain, struct lgw_conf_rxrf_s * conf) {
    ...
    if ((conf->freq_hz < LGW_RF_RX_FREQ_MIN) || (conf->freq_hz > LGW_RF_RX_FREQ_MAX)) {
        DEBUG_PRINTF("ERROR: NOT A VALID RADIO CENTER FREQUENCY, PLEASE CHECK IF IT HAS BEEN GIVEN IN HZ (%lu)\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
    ...
    DEBUG_PRINTF("Note: rf_chain %d configuration; en:%d freq:%ld rssi_offset:%f radio_type:%d tx_enable:%d single_input_mode:%d\n",  rf_chain,
...
int lgw_rxif_setconf(uint8_t if_chain, struct lgw_conf_rxif_s * conf) {
    ...
    if ((conf->freq_hz + ((bw_hz==-1)?LGW_REF_BW:bw_hz)/2) > ((int32_t)rf_rx_bandwidth/2)) {
        DEBUG_PRINTF("ERROR: IF FREQUENCY %ld TOO HIGH\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    } else if ((conf->freq_hz - ((bw_hz==-1)?LGW_REF_BW:bw_hz)/2) < -((int32_t)rf_rx_bandwidth/2)) {
        DEBUG_PRINTF("ERROR: IF FREQUENCY %ld TOO LOW\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
    ...
    switch (sx1302_get_ifmod_config(if_chain)) {
        case IF_LORA_STD:
            ...
            DEBUG_PRINTF("Note: LoRa 'std' if_chain %d configuration; en:%d freq:%ld bw:%d dr:%ld\n", if_chain,
        ...
        case IF_LORA_MULTI:
            ...
            DEBUG_PRINTF("Note: LoRa 'multi' if_chain %d configuration; en:%d freq:%ld\n",   if_chain,
                                                                                            CONTEXT_IF_CHAIN[if_chain].enable,
                                                                                            CONTEXT_IF_CHAIN[if_chain].freq_hz);
            break;
...
/*
            DEBUG_PRINTF("Note: FSK if_chain %d configuration; en:%d freq:%ld bw:%d dr:%ld (%ld real dr) sync:0x%x\n", if_chain,
                                                                                                                        CONTEXT_IF_CHAIN[if_chain].enable,
                                                                                                                        CONTEXT_IF_CHAIN[if_chain].freq_hz,
 ...
 *
 */
            DEBUG_PRINTF("Note: FSK if_chain %d configuration; en:%d freq:%ld bw:%d dr:%ld (%ld real dr) %d\n", if_chain,
                                                                                                                        CONTEXT_IF_CHAIN[if_chain].enable,
                                                                                                                        CONTEXT_IF_CHAIN[if_chain].freq_hz,
                                                                                                                        CONTEXT_FSK.bandwidth,
                                                                                                                        CONTEXT_FSK.datarate,
...
int lgw_start_for_regtest(void) {
    ...
    //reg_stat = lgw_connect_for_regtest(CONTEXT_SPI);
...
int lgw_start(void) {
...
#else
                    DEBUG_MSG("LGW_RADIO_TYPE_SX1250 is not supported in this firmware : you should rebuild it\n");
                    return LGW_HAL_ERROR;
#endif
                    break;
...
#else
                    DEBUG_MSG("LGW_RADIO_TYPE_SX1255 and LGW_RADIO_TYPE_SX1257 are not supported in this firmware : you should rebuild it\n");
                    return LGW_HAL_ERROR;
#endif
                    break;
                default:
    ...
    switch (CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type) {
        ...
        case LGW_RADIO_TYPE_SX1257:
            ...
            break;
        default:
        	DEBUG_PRINTF("Unknown CONTEXT_RF_CHAIN (type=%d)\n", CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type);
            return LGW_HAL_ERROR;
            break;
    }
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    // TODO float current_temperature, rssi_temperature_offset;
    ...
    mutex_lock(&rx_tx_mutex);
    res = sx1302_update();
    mutex_unlock(&rx_tx_mutex);
    if (res != LGW_REG_SUCCESS) {
        return LGW_HAL_ERROR;
    }

    /* Get packets from SX1302, if any */
    //puts("mutex_lock sx1302_fetch");
    mutex_lock(&rx_tx_mutex);
    res = sx1302_fetch(&nb_pkt_fetched);
    mutex_unlock(&rx_tx_mutex);
    //puts("mutex_unlock sx1302_fetch");
    ...
    // TODO res = stts751_get_temperature(ts_fd, ts_addr, &current_temperature);
...
int lgw_send(struct lgw_pkt_tx_s * pkt_data) {
    ...
    mutex_lock(&rx_tx_mutex);
    int res = sx1302_send(CONTEXT_RF_CHAIN[pkt_data->rf_chain].type, &CONTEXT_TX_GAIN_LUT[pkt_data->rf_chain], CONTEXT_LWAN_PUBLIC, &CONTEXT_FSK, pkt_data);
    mutex_unlock(&rx_tx_mutex);
...
uint32_t lgw_time_on_air(struct lgw_pkt_tx_s *packet) {
    ...
    if (packet->modulation == MOD_LORA) {
        ...
        if (val != -1) {
            ...
            DEBUG_PRINTF("ERROR: Cannot compute time on air for this packet, unsupported bandwidth (%d)\n", packet->bandwidth);
            return 0;
        }
        ...
        if (val != -1) {
            ...
            DEBUG_PRINTF("ERROR: Cannot compute time on air for this packet, unsupported datarate (%ld)\n", packet->datarate);
            return 0;
        }
 ...
 */
uint32_t lgw_time_on_air_params(
		uint8_t modulation,
		uint8_t bandwidth, // 0x04 for 125000 KHz
		uint8_t coderate, // 1 for CR4/5
		uint8_t datarate, // SF
		uint8_t preamble,
		bool no_header, /* header is always enabled, except for beacons */
		bool no_crc,
		uint8_t size
) {
    int32_t val;
    ...
    if (modulation == MOD_LORA) {
        ...
        if (val != -1) {
            ...
            DEBUG_PRINTF("ERROR: Cannot compute time on air for this packet, unsupported datarate (%d)\n", datarate);
            return 0;
        }
        ...
        payloadSymbNb = 8 + (ceil((double)(8*size - 4*SF + 28 + 16 - 20*H) / (double)(4*(SF - 2*DE))) * (coderate + 4)); /* Explicitely cast to double to keep precision of the division */
        ...
        /* PREAMBLE + SYNC_WORD + PKT_LEN + PKT_PAYLOAD + CRC
                ...
                CRC: 0 or 2 bytes
        */
        Tfsk = (8 * (double)(preamble + CONTEXT_FSK.sync_word_size + 1 + size + ((no_crc == true) ? 0 : 2)) / (double)datarate) * 1E3;
