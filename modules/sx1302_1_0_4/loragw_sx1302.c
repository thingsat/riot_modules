...
int sx1302_radio_calibrate(struct lgw_conf_rxrf_s * context_rf_chain, uint8_t clksrc, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    if ((context_rf_chain[clksrc].type == LGW_RADIO_TYPE_SX1257) ||
		...
		printf("ERROR: Other radio than SX1255/57	 are not supported : you should rebuild the firmware\n");
        return LGW_REG_ERROR;
    }
#endif
#if ENABLE_SX1250 == 1
    if (context_rf_chain[clksrc].type == LGW_RADIO_TYPE_SX1250) {
		...
		printf("ERROR: Other radio than SX1250 are not supported : you should rebuild the firmware\n");
        return LGW_REG_ERROR;
    }
#endif
...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    ...
    DEBUG_MSG("Write AGC fw in AGC MEM\n");
    lgw_mem_wb(AGC_MEM_ADDR, firmware, MCU_FW_SIZE);

//    /* Read back and check */
//    DEBUG_MSG("Read back and check\n");
...
//    /* Read back and check */
//    DEBUG_MSG("Read back and check\n");
//	uint8_t* fw_check = malloc(MCU_FW_SIZE);
//	// TODO lgw_mem_check_rb(AGC_MEM_ADDR, firmware, MCU_FW_SIZE, false);
//	// In order to avoid malloc(MCU_FW_SIZE);
//	lgw_mem_rb(AGC_MEM_ADDR, fw_check, MCU_FW_SIZE, false);
//	if (memcmp(firmware, fw_check, MCU_FW_SIZE) != 0) {
//		printf ("ERROR: Failed to load AGC fw\n");
//		return LGW_HAL_ERROR;
//	} else {
//		DEBUG_MSG("INFO: AGC fw correctly loaded\n");
//	}
//	free(fw_check);
    ...
    /* Read back and check */
    DEBUG_MSG("Read back and check\n");
	if (lgw_mem_check_rb(AGC_MEM_ADDR, firmware, MCU_FW_SIZE, false)) {
		printf ("ERROR: Failed to load AGC fw\n");
		return LGW_HAL_ERROR;
	} else {
		DEBUG_MSG("INFO: AGC fw correctly loaded\n");
...
int sx1302_arb_load_firmware(const uint8_t *firmware) {
    ...
    DEBUG_MSG("Write ARB fw in ARB MEM\n");
    lgw_mem_wb(ARB_MEM_ADDR, firmware, MCU_FW_SIZE);
...
//    lgw_mem_rb(ARB_MEM_ADDR, fw_check, MCU_FW_SIZE, false);
//	if (memcmp(firmware, fw_check, MCU_FW_SIZE) != 0) {
//		printf ("ERROR: Failed to load ARB fw\n");
//		return LGW_HAL_ERROR;
//	} else {
//		DEBUG_MSG("INFO: ARB fw correctly loaded\n");
//	}
//	free(fw_check);

    /* Read back and check */
    DEBUG_MSG("Read back and check\n");
	if (lgw_mem_check_rb(ARB_MEM_ADDR, firmware, MCU_FW_SIZE, false)) {
		printf ("ERROR: Failed to load AGC fw\n");
		return LGW_HAL_ERROR;
	} else {
		DEBUG_MSG("INFO: AGC fw correctly loaded\n");
...
int sx1302_parse(lgw_context_t * context, struct lgw_pkt_rx_s * p) {
    ...
    if ((ifmod == IF_LORA_MULTI) || (ifmod == IF_LORA_STD)) {
        ...
        if (p->status == STAT_CRC_OK || p->status == STAT_NO_CRC) {
            ...
            */
            int res;
            for (int i = 0; i < context->debug_cfg.nb_ref_payload; i++) {
                res = dbg_check_payload(&(context->debug_cfg), log_file, p->payload, p->size, i, pkt.rx_rate_sf);
                if (res == -1) {
                    printf("ERROR: 0x%08lX payload error\n", context->debug_cfg.ref_payload[i].id);
                    //if (log_file != NULL) {
                        //fprintf(log_file, "ERROR: 0x%08lX payload error\n", context->debug_cfg.ref_payload[i].id);
                    ...
                    DEBUG_PRINTF("0x%08X payload matches\n", (unsigned int)context->debug_cfg.ref_payload[i].id);
        ...
        //printf("Note: FSK packet (modem %u chan %u)\n", pkt.modem_id, p->if_chain);
...
uint8_t sx1302_tx_status(uint8_t rf_chain) {
    ...
    if (read_value == 0x80) {
        ...
        printf("ERROR: UNKNOWN TX STATUS 0x%02lX\n", read_value);
        return TX_STATUS_UNKNOWN;
    }
}
...
int sx1302_send(lgw_radio_type_t radio_type, struct lgw_tx_gain_lut_s * tx_lut, bool lwan_public, struct lgw_conf_rxif_s * context_fsk, struct lgw_pkt_tx_s * pkt_data) {
    ...
    switch (pkt_data->tx_mode) {
        ...
        case TIMESTAMPED:
            ...
            DEBUG_PRINTF("--> programming trig delay at %lu (%lu)\n", pkt_data->count_us - (tx_start_delay / 32), count_us);
