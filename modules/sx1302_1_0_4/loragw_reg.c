...
//int lgw_connect_for_regtest(const char * spidev_path) {
...
//    spi_stat = lgw_spi_rb(lgw_spi_target, LGW_SPI_MUX_TARGET_SX1302, loregs[SX1302_REG_COMMON_VERSION_VERSION].addr, &u, 1);
...
int lgw_reg_w(uint16_t register_id, int32_t reg_value) {
    DEBUG_RW("[%s]: reg=%d value=%ld\n", __FUNCTION__, register_id, reg_value);
...
int lgw_reg_r(uint16_t register_id, int32_t *reg_value) {
    DEBUG_RW("[%s]: ENTER reg=%d\n", __FUNCTION__, register_id);
    ...
    DEBUG_RW("[%s]: EXIT reg=%d value=%ld\n", __FUNCTION__, register_id, *reg_value);
...
int lgw_reg_wb(uint16_t register_id, uint8_t *data, uint16_t size) {
    DEBUG_RW("[%s]: reg=%d size=%d\n", __FUNCTION__, register_id, size);
...
const uint16_t CHUNK_SIZE_MAX = 32;

/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

int lgw_mem_wb(uint16_t mem_addr, const uint8_t *data, uint16_t size) {
    DEBUG_RW("[%s]: addr=0x%x size=%d\n", __FUNCTION__, mem_addr, size);
...
int lgw_mem_check_rb(uint16_t mem_addr, const uint8_t *data_to_check, uint16_t size, bool fifo_mode) {
    DEBUG_RW("[%s]: addr=0x%x size=%d\n", __FUNCTION__, mem_addr, size);
    ...
    while (sz_todo > 0) {
        ...
        /* do the burst read */
        spi_stat += lgw_spi_rb(lgw_spi_target, LGW_SPI_MUX_TARGET_SX1302, addr, buf, chunk_size);

		if (memcmp(data_to_check + (chunk_cnt * CHUNK_SIZE_MAX), buf, chunk_size) != 0) {
	        DEBUG_MSG("ERROR: READEN FIRMWARE MISMATCHED WITH WRITTEN FIRMWARE\n");
	        return LGW_REG_ERROR;
		}
