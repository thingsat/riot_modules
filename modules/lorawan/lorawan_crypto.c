...
void LoRaMacPayloadEncrypt( const uint8_t *buffer, uint16_t size, const uint8_t *key, uint32_t address, uint8_t dir, uint32_t sequenceCounter, uint8_t *encBuffer )
{
    ...
    semtech_aes_set_key( key, 16, &AesContext );
    ...
    while( size >= 16 )
    {
        aBlock[15] = ( ( ctr ) & 0xFF );
        ctr++;
        semtech_aes_encrypt( aBlock, sBlock, &AesContext );
        for( i = 0; i < 16; i++ )
        {
...
void LoRaMacJoinDecrypt( const uint8_t *buffer, uint16_t size, const uint8_t *key, uint8_t *decBuffer )
{
    memset1( AesContext.ksch, '\0', 240 );
    semtech_aes_set_key( key, 16, &AesContext );
    semtech_aes_encrypt( buffer, decBuffer, &AesContext );
    // Check if optional CFList is included
    if( size >= 16 )
    {
    	semtech_aes_encrypt( buffer + 16, decBuffer + 16, &AesContext );
    }
}
...
void LoRaMacJoinComputeSKeys( const uint8_t *key, const uint8_t *appNonce, uint16_t devNonce, uint8_t *nwkSKey, uint8_t *appSKey )
{
    ...
    semtech_aes_encrypt( nonce, nwkSKey, &AesContext );

    memset1( nonce, 0, sizeof( nonce ) );
    nonce[0] = 0x02;
    memcpy1( nonce + 1, appNonce, 6 );
    memcpy1( nonce + 7, pDevNonce, 2 );
    semtech_aes_encrypt( nonce, appSKey, &AesContext );
}
...
{
	LoRaMacComputeMic(frame_buffer, frame_size, nwkskey, devaddr, dir, sequenceCounter, mic);
}
...
{
	LoRaMacPayloadEncrypt(payload_buffer, payload_size, key, devaddr, dir, sequenceCounter, encrypt_buffer);
}
...
{
	LoRaMacPayloadDecrypt(payload_buffer, payload_size, key, devaddr, dir, sequenceCounter, decrypt_buffer);
}
